...
void PrintDASHUsage()
{
	fprintf(stderr, "DASH Options:\n"
	        ...
	        " -renum-tracks        renumbers track IDs starting at 1.\n"
...
Bool mp4box_parse_args(int argc, char **argv)
{
	...
	for (i = 1; i < (u32)argc; i++) {
		...
		else if (!stricmp(arg, "-renum-tracks")) {
			renum_tracks = 1;
		}
...
int mp4boxMain(int argc, char **argv)
{
	...
	if (dash_duration) {
		...
		if (!e) e = gf_dasher_enable_renum_tracks(dasher, renum_tracks);
