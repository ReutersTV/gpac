	...
	for (i=0; i<count; i++) {
		...
		for (j=0; j<=max_layer_id; j++) {
			...
			if (! sti[j].track_num) {
				...
				e = gf_isom_clone_track(file, track, file, GF_FALSE, GF_FALSE, &sti[j].track_num);
				if (e) goto exit;
...
GF_Err gf_media_split_hevc_tiles(GF_ISOFile *file, Bool use_extractors)
{
	...
	for (i=0; i<nb_tracks; i++) {
		e = gf_isom_clone_track(file, track, file, GF_FALSE, GF_FALSE, &tiles[i].track );
		if (e) goto err_exit;
...
GF_Err gf_media_fragment_file(GF_ISOFile *input, const char *output_file, Double max_duration_sec)
{
	...
	for (i=0; i<gf_isom_get_track_count(input); i++) {
		e = gf_isom_clone_track(input, i+1, output, GF_FALSE, GF_FALSE, &TrackNum);
		if (e) goto err_exit;
