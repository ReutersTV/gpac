...
//	u32 first_seg;
	...
	for (i=0; i<gf_isom_get_track_count(input); i++) {
		...
		if (!dash_moov_setup) {
			e = gf_isom_clone_track(input, i+1, output, GF_FALSE, dash_cfg->renum_tracks, &TrackNum);
			if (e) goto err_exit;
...
retry_track:
				...
				gf_isom_clone_track(in, j+1, init_seg, GF_FALSE, dash_opts->renum_tracks, &track);
...
GF_EXPORT
GF_Err gf_dasher_enable_renum_tracks(GF_DASHSegmenter *dasher, Bool enable)
{
	if (!dasher) return GF_BAD_PARAM;
	dasher->renum_tracks = enable;
	return GF_OK;
}
